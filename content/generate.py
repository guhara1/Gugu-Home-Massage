...
def visit_type(character: str, slug: str) -> str:
    ...
    if any(k in c for k in ["오피스텔", "업무", "IT", "R&D", "법조", "비즈니스", "금융", "단지권"]):
        return ("오피스텔과 비즈니스호텔 방문 수요가 많아, 평일 저녁과 심야 시간대 "
                "예약이 비교적 많은 편입니다.")
    if any(k in c for k in ["상권", "관광", "숙박", "호텔", "유흥", "먹자"]):
        return ("주변에 호텔·숙소가 많아 자택뿐 아니라 숙소 방문 이용이 함께 "
                "나타나는 지역입니다.")
    if any(k in c for k in ["대학", "원룸", "1인", "고시"]):
        return ("원룸·소형 주거와 숙소가 섞여 있어, 1인 가구 자택 방문 수요가 "
                "두드러집니다.")
    if any(k in c for k in ["고급", "명품", "레지던스", "타워"]):
        return ("프라이버시를 중시하는 자택·레지던스 방문이 많아, 정확한 출입 안내가 "
                "특히 중요합니다.")
    return ("아파트·주택 등 자택 방문 수요가 안정적으로 이어지는 지역으로, 정확한 "
            "주소와 출입 방식 확인이 중요합니다.")
...
def station_body(s) -> str:
    ...
    parts.append(
        f"<section><p>{intro} {s.get('character','')} 이 페이지는 {name} 기준으로 "
        f"인접한 행정동, 행정구, 생활권을 정리해 방문 예약 전 위치를 빠르게 "
        f"확인할 수 있도록 안내합니다. 환승역도 노선·출구별로 나누지 않고 역명 "
        f"기준 한 페이지로 안내합니다.</p></section>"
    )
    ...
    parts.append(
        f"<section><h2>{name} 인접 행정동</h2>"
        f"<p>{name}에서 가까운 행정동입니다. 같은 역세권 동선으로 방문이 이어지는 "
        f"지역을 함께 안내합니다. 아래 행정동 페이지에서는 각 동의 인접 역과 생활권을 "
        f"더 구체적으로 확인할 수 있습니다.</p><ul>{dlinks}</ul></section>"
    )
    ...
    parts.append(
        ...
        f"<p>{name}은 {glinks} 생활권과 이어집니다. 대표 생활권은 {life_html}"
        f" 권역으로, 업무지구·상권·주거가 함께 분포합니다.</p></section>"
    )

    parts.append(
        f"<section><h2>{name} 방문 동선과 이용 형태</h2>"
        f"<p>{visit_type(s.get('character',''), slug)} {name}은 위치를 좁히는 "
        f"기준일 뿐, 실제 방문은 역이 아니라 자택·오피스텔·호텔의 건물 주소를 "
        f"기준으로 진행됩니다. 가까운 역과 함께 정확한 주소·동·호수를 알려주시면 "
        f"도착 시간과 동선이 정확해집니다.</p>"
